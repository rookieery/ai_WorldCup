"""Alembic env.py — async migration runner for football-server.

Reads the database URL from ``app.config.settings`` so that the value
specified in ``.env`` (or the default) is always used.  Runs migrations
asynchronously via ``aiosqlite``.
"""
...
# Set up Python logging from alembic.ini if the config section exists.
...
def do_run_migrations(connection) -> None:
    """Shared helper that actually runs migrations within a connection."""
    context.configure(

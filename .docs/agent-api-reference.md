...
| `MatchService` | `get_match_dates(timezone_name)`、`get_matches(params, timezone_name, lang, page, page_size)`、`get_match_by_id(match_id, timezone_name, lang)`、`get_live_matches(timezone_name, lang)`、`find_match_context(team1_name, team2_name, lang)` | 多条件筛选（日期/阶段/小组/球队/状态）含二次内存过滤（时区感知）；通过 `zoneinfo` 进行时区转换，添加 `local_time` 和 `host_time` 字段；`get_match_dates` 返回不同日期及主阶段标签（支持时区分组）；**日期筛选支持时区感知**：传入 `timezone_name` 时按用户本地日期过滤，否则按 UTC；语言感知（为球队提升 `name_zh`，为场馆提升 `name_zh`/`city_zh`/`country_zh`）；**Redis 可用时自动合并实时数据**（status/score/activity_level）到查询结果；**`find_match_context`**：通过 `TeamRepository.search_by_name()` 解析队名 → `MatchRepository.find_by_team_names()` 定位比赛 → 返回结构化 dict（external_id、group、round、status、主客队详情含 fifa_ranking/confederation/world_cup_appearances、小组球队列表），为飞书 Bot 提供 DB 真实数据注入 |
...
| `PromptBuilder` | `build_system_prompt(lang)`、`build_match_analysis_prompt(match_id, team1, team2, lang)`、`build_custom_match_analysis_prompt(match_id, team1, team2, lang, match_context?)`、`build_knockout_prompt(match_id, team1, team2, lang)`、`build_chat_context(messages)`、`resolve_skill_id(skill_id, stage)`、`get_available_skills()`、`build_skill_prompt(request: MatchAnalysisRequest)`、`build_championship_prompt(request: ChampionshipAnalysisRequest)`、`_format_feishu_match_context(match_context)` | AI 提示词构建器；`_SKILL_REGISTRY` 模块级字典映射 4 个 skill_id（group_stage_predict、group_stage_round_strategy、knockout_stage_predict、championship_predict）；`build_custom_match_analysis_prompt` 加载 `group_stage_round_strategy.md` skill + `custom_analysis_intro` 模板（R1爆冷猎手/R2稳定猎手/R3终局猎手），可选 `match_context` 参数注入真实 DB 数据（队名、match_id 从 context 提取）；`_format_feishu_match_context` 静态方法将结构化 match_context dict 格式化为提示词段落（含小组、轮次、FIFA 排名、大洲、参赛次数、小组球队列表）；`build_championship_prompt` 加载冠亚军策略文件（对 `{simulation_count}` 占位符做动态替换）+ 蒙特卡洛模拟指令（`simulation_count` 从 `request.simulation_count` 读取，动态注入 prompt）→ `[system_msg, user_msg]`；提示词常量抽取至 `app/services/prompts/` 子模块（system_prompts.py、championship_prompts.py）；所有方法为 static；无实例状态 |
...
| Repository | 额外方法 | 说明 |
|------------|----------|------|
| `TeamRepository` | `get_by_code(code)`、`get_by_group(group_label)`、`search_by_name(name_query)` | code 不存在时抛出 `NotFoundError`；`search_by_name` 按 code 不区分大小写、name ILIKE 模糊匹配、name_zh 精确匹配搜索球队 |
| `MatchRepository` | `get_by_date(date)`、`get_by_stage(stage)`、`get_by_status(status)`、`get_live_matches()`、`get_bracket_matches()`、`get_by_group_label(group)`、`get_by_team_code(code)`、`get_match_dates()`、`find_by_team_names(team1_name, team2_name)` | 除 `get_live_matches` / `get_bracket_matches` / `get_match_dates` / `find_by_team_names` 外均分页；`find_by_team_names` 双向匹配主客队名称查找小组赛比赛，优先返回未开赛比赛 |

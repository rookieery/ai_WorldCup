...
执行顺序（v1.2）：
  R1：     STEP 0 → 1 → 2 → 3 → 5 → 5.5（跳过 0.5/3.5/4/6，首轮无历史数据与路径博弈基础）
  R2/R3：  STEP 0 → 0.5（数据源加载，消费 data md）→ 1 → 2 → 6（路径预计算，前置）→ 3 → 3.5（名次价值×体力，消费 step6）→ 4（仅2026）→ 5 → 5.5
注：STEP 6 在 R2/R3 提前执行以为 STEP 3.5 提供 path_diff；其完整方法论见末尾"STEP 6"章节。
...
### STEP 0.5: 历史轮次数据源加载 (Historical Round Data Loading)

> **v1.2 新增**。**条件化执行**：R2/R3 前瞻预测时执行（R1 跳过——首轮无历史数据）。目标：将 data 数据文件中**已完赛轮次的实际结果与官方积分快照**结构化喂入推理链，替代人工填写 `completed_matches`。
...
> **回灌**：`group_completed_matches` → 覆盖 input 的 `group_context.completed_matches`；`pre_match.points_*` → STEP 3.1 ASS 公式的 points 输入；`pre_match.standings_rank_*` → STEP 3.5.2 的 current_seed 判定（**不**覆盖 rank_group）。
...
> **数据来源（v1.2）**：R2/R3 的 `points` 优先取 STEP 0.5 从 data md 读取的赛前积分快照（官方值，已处理同组同时开赛）；STEP 0.5 未执行或降级时，从 input 的 completed_matches 自行累加。注意 `rank_group` 仍取 FIFA 排名序号（静态实力档位），与 md 的「赛前排名」（积分榜动态排名）语义不同，不得互相替代。
...
5. **数据文件依赖（v1.2）**: R2/R3 前瞻预测时，STEP 0.5 需读取 `data/2026_FIFA_World_Cup_Group_Stage.md`（人工维护的赛程+结果真相源）。**维护责任**：每轮完赛后须将该组表格的「90分钟」「赛前积分」「赛前排名」更新为官方实际值——这是 R2/R3 预测准确性的硬前提，数据滞后或错误会直接污染 ASS、出线状态、路径分析整条链。

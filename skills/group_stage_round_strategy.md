...
- **v1.4 新增**：打通淘汰赛对手推理链，修复 ISSUE-002（R2/R3 此前无法获取/推理真实淘汰赛对手）。三处改造：①新增「2026 淘汰赛对阵结构参考」内嵌章节（半区速查/R32对阵/第三名约束/第三轮时间线，镜像权威源 `skills/冠亚军分析.md` 2.1-2.5）；②STEP 0.5 新增「阶段三·跨组出线快照与对手图」（仅 R2/R3，把阶段二的原始跨组比分聚合为出线格局，join 对阵结构产出 `knockout_opponent_graph`）；③STEP 6 从"文字引用外部表+概率裸估算"重写为"消费 opponent_graph 的结构化对手推理"，新增正式 `confidence` 字段（DETERMINED/CANDIDATE_SET/THIRD_POOL 三态分级 + R3 info_clarity 折损）。配套：STEP 3.5.2 的 SV 阈值随 confidence 联动、SEALED_TANK 触发收紧（需路径偏好可信），STEP 5.3 的 knockout_path_bias 按 confidence 折损。核心动机：让"锁定球队放不放水"的判断建立在可信的对手评估上，而非不可靠的估算。
- **v1.5 新增**：重写 STEP 4（第3名出线概率评估），从 v1.0 的"固定门槛表（4分>95%/3分~50%/2分20-30%/1分<5%）"升级为"消费 STEP 0.5 阶段三 `group_standings_snapshot` 的跨组实时第3名排名对比"，对齐 v1.4 对 STEP 6 的同款改造。核心新增：①**实时出线门槛**推算（`advancement_threshold_points`，门槛表给不出的"本届几分能出线"）；②**落第3情景枚举**（9 组合确定性推演本队是否落第3）；③**第3名池排名对比**（替代门槛表查表）；④**confidence 分级 + info_clarity 联动**（R2/迷雾组输出概率区间，R3 透明组接近精确）；⑤**impact_on_match 按 confidence 折损**（防止不可信信息上做强动机修正）。核心动机：第3名出线是 12 组第3名的相对竞争，固定门槛表在本届低分扎堆时系统性失准——修复 v1.4 改造 STEP 6 时遗漏的 STEP 4 同源问题（阶段三早已声明把 candidates_3rd 回灌 STEP 4，但 STEP 4 方法论此前未兑现）。
...
## 2026 淘汰赛对阵结构参考（镜像权威源）

> **镜像声明**：本节同步镜像自 `skills/冠亚军分析.md` 2.1-2.5，**冠亚军分析为权威源**。任何对阵结构变更必须双向同步。STEP 6（淘汰赛路径博弈）每场查表依赖本节；STEP 0.5 阶段三据此构建 `knockout_opponent_graph`。

> **数据性质**：以下对阵结构为 FIFA 官方抽签确定的**静态骨架**（哪个小组第几名去哪个 R32 槽位），不随小组赛进程变化。变化的只是每个槽位上**实际坐的是谁**——这部分由 STEP 0.5 阶段三的跨组出线快照动态填充。同组第1名与第2名被强制分入不同半区，只有决赛才能相遇。

### 半区分布速查

> 查表第一步：本场球队若排第1/第2，去哪个 R32 槽位、哪个半区。下=下半区(SF_02 路径)，上=上半区(SF_01 路径)。

| 小组 | 第1名 → R32(半区) | 第2名 → R32(半区) | 小组 | 第1名 → R32(半区) | 第2名 → R32(半区) |
|------|------------------|------------------|------|------------------|------------------|
| A | R32_07 (下) | R32_01 (上) | G | R32_09 (上) | R32_14 (下) |
| B | R32_13 (下) | R32_01 (上) | H | R32_11 (上) | R32_15 (下) |
| C | R32_02 (下) | R32_04 (上) | I | R32_06 (上) | R32_05 (下) |
| D | R32_10 (上) | R32_14 (下) | J | R32_15 (下) | R32_11 (上) |
| E | R32_03 (上) | R32_05 (下) | K | R32_16 (下) | R32_12 (上) |
| F | R32_04 (上) | R32_02 (下) | L | R32_08 (下) | R32_12 (上) |

### R32 对阵明细

> 查表第二步：定位到的 R32 槽位，对手是谁。仅展开至 R32 层（小组赛推理 R32 首轮对手所需），R16/QF/SF 完整树见 `skills/冠亚军分析.md` 2.1/2.2。

**上半区（SF_01 路径 → 决赛 pos=1）**

| R32 比赛 | 对阵 | → R16 |
|---------|------|-------|
| R32_03 (M74) | E1 vs 3rd(A/B/C/D/F) | R16_01 |
| R32_06 (M77) | I1 vs 3rd(C/D/F/G/H) | R16_01 |
| R32_01 (M73) | A2 vs B2 | R16_02 |
| R32_04 (M75) | F1 vs C2 | R16_02 |
| R32_12 (M83) | K2 vs L2 | R16_05 |
| R32_11 (M84) | H1 vs J2 | R16_05 |
| R32_10 (M81) | D1 vs 3rd(B/E/F/I/J) | R16_06 |
...
| R32 比赛 | 对阵 | → R16 |
|---------|------|-------|
| R32_02 (M76) | C1 vs F2 | R16_03 |
| R32_05 (M78) | E2 vs I2 | R16_03 |
| R32_07 (M79) | A1 vs 3rd(C/E/F/H/I) | R16_04 |
| R32_08 (M80) | L1 vs 3rd(E/H/I/J/K) | R16_04 |
| R32_15 (M86) | J1 vs H2 | R16_07 |
| R32_14 (M88) | D2 vs G2 | R16_07 |
| R32_13 (M85) | B1 vs 3rd(E/F/G/I/J) | R16_08 |
| R32_16 (M87) | K1 vs 3rd(D/E/I/J/L) | R16_08 |

> **对手类型速辨**：对阵含 `3rd(...)` → 本方若拿该名次，对手是**动态第三名**（身份未定，走 THIRD_POOL 分支）；对阵为 `X1/X2 vs Y1/Y2` → 对手是**确定性槽位**（走 DETERMINED/CANDIDATE_SET 分支，查对方组出线快照）。

### 第三名候选来源约束

> 8 场涉及第三名的 R32 比赛，第三名候选来源组（均**回避种子队同组**）。STEP 6 推理 THIRD_POOL 对手池时，按此约束圈定候选来源组的第3名候选。
...
> 同组同时开赛（R3 第3轮两组同开）意味着预测本场时，其他组本场结果未知。本组在时间线中的位置决定**淘汰赛落位情报的透明度**——越靠后完赛，控分动机越强。R3 的 SEALED_TANK（隐性放水选半区）只在信息透明时可信。
...
| 小组 | 时间线序位(group_index) | info_clarity | 信息状态 |
|------|------------------------|--------------|---------|
| B | 0 | 0.00 | 迷雾(≤0.5) |
| C | 1 | 0.09 | 迷雾(≤0.5) |
| A | 2 | 0.18 | 迷雾(≤0.5) |
...
- `info_clarity = group_index / 11.0`
- `info_clarity ≤ 0.5`（前6组 B/C/A/E/F/D）：**信息迷雾**——其他组本场结果未出、淘汰赛落位不透明，confidence 强制降一档，禁止强控分推演。
- `info_clarity > 0.5`（后6组 I/H/G/L/K/J）：**信息透明**——落位情报基本明朗，允许强控分推演（承接冠亚军分析策略七）。
...
执行顺序（v1.4）：
  R1：     STEP 0 → 0.5(仅阶段一: FIFA排名) → 1 → 2 → 3 → 5 → 5.5（跳过 0.5阶段二/三/3.5/4/6，首轮无历史数据与路径博弈基础）
  R2/R3：  STEP 0 → 0.5(阶段一: FIFA排名 + 阶段二: 历史轮次 + 阶段三: 出线快照与对手图) → 1 → 2 → 6（对手推理+路径预计算，前置）→ 3 → 3.5（名次价值×体力，消费 step6 的 path_diff/confidence）→ 4（仅2026）→ 5 → 5.5
注1：STEP 0.5 阶段一（FIFA 排名加载）全轮次执行——STEP 1 的 rank_diff 与 STEP 2 的爆冷模式匹配都依赖它，R1 不能跳过；阶段二（历史轮次）与阶段三（出线快照与对手图）均仅 R2/R3 执行。
注2：STEP 6 在 R2/R3 提前执行，消费阶段三的 opponent_graph 推理 R32 对手，为 STEP 3.5 提供 path_diff + confidence；其完整方法论见末尾"STEP 6"章节。
...
#### 阶段三 · 跨组出线快照与对手图 (Cross-Group Standings Snapshot & Opponent Graph) — 仅 R2/R3

> **v1.4 新增**。**条件化执行**：R2/R3（R1 跳过——首轮无跨组出线信息可聚合，也无路径博弈基础）。

> **解决的问题**：阶段二产出的是**原始跨组比分**（`other_groups_completed_matches`），STEP 6 无法直接消费。阶段三把原始比分**聚合**为"各组当前出线格局"，再与上方「淘汰赛对阵结构参考」**join** 成 `knockout_opponent_graph`——回答"本场球队若排第1/第2，R32 首轮将遇到谁、对手有多强"。这是 STEP 6 从"概率性裸估算"升级为"基于真实出线快照的对手推理"的数据前提。

**3.1 聚合 12 组出线快照 (Standings Snapshot)**

遍历 data md 全部 12 组（A-L）的已完成行 + 赛前积分快照（阶段二已加载），按"积分 → 净胜球（0.5.3 自行累加）→ 进球"为每组推算当前格局：

```json
group_standings_snapshot = {
  "A": { "ranked": ["墨西哥","韩国",...], "locked": {"1st": true, "2nd": false, "seed_locked": ...},
         "candidates_3rd": [...], "points": {...} },
  ... // 12 组
}
```

- **locked 判定**：复用 STEP 3.5.2 的 seed_locked 逻辑（积分+净胜球使对手无法反超）。第1/第2已锁 → `locked.1st/2nd = true` 并填具体队名；未锁 → 该名次标为「候选集」（按当前积分排序的 1-3 支候选队）。
- **candidates_3rd**：该组当前暂列第3的球队（若该组已产生明确第3名候选则具体队，否则为候选集）。

**3.2 构建对手图 (Opponent Graph)**

为本场所在组的「第1落点」「第2落点」两个 R32 槽位（查上方半区分布速查表确定）逐一标注对手：

| 对手槽位类型 | 判定条件 | opponent 内容 | STEP 6 confidence 贡献 |
|------------|---------|--------------|----------------------|
| `DETERMINED` | 对阵为 `X1/X2 vs Y1/Y2`（确定性槽位）**且**对方组该名次 `locked=true` | 具体队名 + FIFA 排名 | HIGH |
| `CANDIDATE_SET` | 确定性槽位但对方组该名次未锁 | 候选队集 + FIFA 排名区间 | MEDIUM |
| `THIRD_POOL` | 对阵含 `3rd(...)`（第三名槽位，身份恒未定至小组赛全部结束） | 候选来源组的第3名候选池 + 概率性实力区间 | LOW |

- **对手 FIFA 排名**取自 data md「标注说明」的 48 队排名汇总表（或对方小组表格「FIFA排名(主/客)」列），与阶段一同源（开赛前静态排名）。
- **THIRD_POOL 特赦风险**：候选池内若含 FIFA 排名 ≤ 10 的强队（死亡之组第三名），标注 `strong_third_risk=true`，STEP 6 据此上调路径难度。
- **半区记忆**：本组第1与第2分属不同半区（对阵结构强制），两条路径的对手互不影响。

**3.3 输出**

```json
{
  "phase": "opponent_graph",
  "source": "data/2026_FIFA_World_Cup_Group_Stage.md 跨组聚合 + 内嵌对阵结构",
  "group_standings_snapshot": "{12组的ranked/locked/candidates_3rd}",
  "knockout_opponent_graph": {
    "as_1st": {
      "r32_slot": "R32_07",
      "half": "下半区|上半区",
      "opponent_type": "THIRD_POOL | DETERMINED | CANDIDATE_SET",
      "opponent_team": "string|null (DETERMINED时)",
      "opponent_fifa_rank": "int|null (DETERMINED时)",
      "opponent_candidate_set": ["string"] ,  // CANDIDATE_SET时
      "opponent_pool": ["C3","E3",...],        // THIRD_POOL时（候选来源组的第3名候选）
      "opponent_strength_range": [r_min, r_max],// CANDIDATE_SET/THIRD_POOL时
      "strong_third_risk": "boolean (THIRD_POOL时)"
    },
    "as_2nd": { /* 同结构 */ }
  }
}
...
> **回灌**：`knockout_opponent_graph.as_1st/as_2nd` → STEP 6 推理 `path_diff_as_1st/path_diff_as_2nd` 与 `confidence` 的权威输入；`group_standings_snapshot` → STEP 4（v1.5 起真实消费 candidates_3rd 做跨组实时第3名排名对比，不再用固定门槛表）与 STEP 6（对手候选池）。阶段三**不**回灌本队积分/排名（那些由阶段二的 `pre_match.*` 负责）。
...
**v1.1 新增**。**条件化执行**：R2/R3 执行（R1 不执行——首轮无路径博弈基础）。**前置依赖**：STEP 6 的 `path_diff`（SV）与 `confidence`（路径分析已前置，见 STEP 6 触发条件）——v1.4 起 STEP 6 基于 STEP 0.5 阶段三的 opponent_graph 推理真实 R32 对手，SV 阈值与子态判定随 confidence 联动（见 3.5.2）。
...
子态判定（v1.4：SV 阈值与 TANK 触发随 confidence / info_clarity 联动）:
  // 路径可信度调节 SV 判定阈值：confidence 低 → 放宽 NEUTRAL，避免把对手估算噪音误判为名次有价值
  sv_threshold = 0.2 (confidence=HIGH) | 0.3 (MEDIUM) | 0.4 (LOW)

  IF seed_locked == TRUE:
    → SEALED_SEED_SET     urgency=0.45, intensity=LIGHT      // 名次已定，本场无关排名，轻装
  ELIF |SV| < sv_threshold:                                  // 两路径难度差异不可信 / 相当
    → SEALED_NEUTRAL      urgency=0.12, intensity=ROTATION   // 名次无差异，放水轮换
  ELIF 本场输/平能落到更优名次(SV符号有利) AND 单边动机
       AND 路径偏好可信 (confidence ≥ MEDIUM AND (本轮 ≠ R3 OR info_clarity > 0.5)):
    → SEALED_TANK         urgency=0.10, intensity=PASSIVE    // "不想赢"以选半区（隐性放水，需高可信）
  ELSE:                                                      // |SV| ≥ sv_threshold，名次有价值且本场胜负决定排名
...
> **v1.4 改造（修复 ISSUE-002 污染链根因）**：原版 `|SV|<0.2` 固定阈值在 `path_diff` 不可信时会把对手估算噪音误判为"名次有价值/无差异"，且 TANK（隐性放水选半区）在路径情报不透明时也会误触发。现引入 `sv_threshold` 随 STEP 6 `confidence` 反向调节（confidence 低 → 阈值放宽 → 更易判 NEUTRAL/轻装），并对 TANK 追加"路径偏好可信"前置门（R2 因 confidence 多为 LOW 几乎不触发；R3 仅后 6 组 `info_clarity>0.5` 且 `confidence≥MEDIUM` 才触发）。核心原则：**对手估算不可靠时，SEALED 默认走保守的 SEED_SET/NEUTRAL，而非激进的 TANK/SEED_MATTERS**——避免在不可信的 path_diff 上做出"放水/认真争"的高风险判定。
...
**v1.5 重写**：从 v1.0 的"固定门槛表（4分>95% / 3分~50% / 2分20-30% / 1分<5%）"升级为"消费 STEP 0.5 阶段三 `group_standings_snapshot` 的跨组实时第3名排名对比"，对齐 v1.4 对 STEP 6 的同款改造。结束 STEP 4 此前作为"无视本届实际第3名竞争态势的概率裸估算"盲区。**核心动机**：第3名出线是 12 组第3名的**相对竞争**（取前8），"几分能出线"取决于当时第3名池中已有几个≥4分/≥3分且净胜球更高——固定门槛表在本届低分第3名扎堆时系统性失准（同样3分，池中已8个≥4分则≈0%出线，池中普遍≤2分则≈90%出线，门槛表的"3分~50%"两者皆错）。

**前置依赖**：STEP 0.5 阶段三的 `group_standings_snapshot`（12 组 ranked/locked/candidates_3rd/points）+ 阶段二 `pre_match`（本场双方积分）+ 0.5.3 自行累加的净胜球(gd)/进球(gf)。

#### 4.1 落第3情景枚举（本组，确定性推演）

对本场双方，枚举本场结果（胜/平/负）× 同组另一场 R3 结果（主胜/平/客胜）= 9 个组合。每个组合下：

```
本队最终积分 = pre_match.points + {3 胜 / 1 平 / 0 负}
同组4队最终 (points, gd, gf) 确定 → 排名确定 → 本场双方是否落到本组第3名 确定
对落到第3的组合，记录该队的 (final_points, final_gd, final_gf)
```

> 同组 R3 两组同开，另一场结果未知——9 组合覆盖其全部可能，4.5 出线概率合成时按各组合概率加权。

#### 4.2 实时出线门槛推算（本届第3名池）

聚合 12 组第3名候选（`group_standings_snapshot.candidates_3rd`）构建第3名池，排序定位第8名（最后出线位）：

```
third_place_pool = []
FOR each group G in A-L:
  IF G.locked.3rd == true（该组第3已锁）→ 取锁定第3名的 (points, gd, gf)
  ELSE → 取领先候选的 (points, gd, gf)（候选集则取积分区间中位）
  → 加入 third_place_pool
按 (points > gd > gf) 降序排序
advancement_threshold_points = third_place_pool[8th].points   // 本届实时出线门槛
```

> **门槛表给不出的核心量**：直接回答"本届几分能出线"。例：R2 后若第8名第3名当前=2分 → 门槛=2分（3分稳进，远高于门槛表"3分~50%"）；若第8名=4分 → 门槛=4分（3分大概率淘汰，远低于门槛表"3分~50%"）。这是 v1.5 相对固定门槛表最大的精度跃升。

#### 4.3 本队落第3排名对比

对每个落第3情景（4.1 产出），将本队的 (final_points, final_gd, final_gf) **替换**池中"本组第3"占位后重排，计算该情景下的排名 `rank_in_pool`（1-12）：

```
rank ≤ 8                          → 该情景出线 (P_advance = 1.0)
rank = 9 AND points >= threshold  → 净胜球/进球 tiebreaker 决定 (P_advance = 0.3-0.5)
points < threshold - 1            → 大概率淘汰 (P_advance = 0.0-0.1)
其余                              → 线性插值 (P_advance = 0.1-0.9)
```

#### 4.4 不确定性处理（confidence 联动，对齐 STEP 6）

其他组第3名的最终积分随 R3 结果变化，按信息透明度分级。**核心原则**：信息迷雾时不强装精确，输出概率区间而非点估（与 STEP 6 的 confidence 哲学一致）。

| 执行轮次 / info_clarity | 其他组第3名确定性 | confidence | 排名对比方式 |
|------------------------|------------------|-----------|-------------|
| R2（仅1轮数据） | 极低（11组第3名几乎全是候选集） | LOW | 仅推算实时门槛 + 本队情景，概率输出为宽区间 |
| R3 前6组（info_clarity≤0.5，迷雾） | 低（多数组本场未赛） | LOW/MEDIUM | 区间排名（最坏/最好名次），概率折中 |
| R3 后6组（info_clarity>0.5，透明） | 高（多数组 R3 已出结果） | MEDIUM/HIGH | 接近精确排名，概率可信 |

#### 4.5 出线概率合成

```
home_third_place_probability = Σ_{r∈{W,D,L}} Σ_{m∈{W,D,L}(另一场)}
    P(本场=r) × P(另一场=m) × I(home落第3 in (r,m)) × P(home排进前8 | home落第3)
// away 同理
// P(本场=r) 取 STEP 5 初步三分概率；P(另一场=m) 取该场 STEP 1 实力基线概率（rank_diff 查表）
// P(排进前8) 由 4.3 推算
// confidence=LOW 时，输出 [p_low, p_high] 区间（p_low=最坏排名情景，p_high=最好排名情景）
```

**影响**（回灌 STEP 3/5，**v1.5：按 confidence 折损**）：
- 一方输球仍可第3名出线（实时门槛低）→ urgency 降低，MAF 增大
- 双方输球都淘汰（门槛高 + 本队积分不足）→ 极端保守或鱼死网破
- 双方都有后路（门槛低）→ 偏开放
- **折损规则**：`impact_strength = base_impact × confidence_factor`（HIGH=1.0 | MEDIUM=0.6 | LOW=0.3）——R2/迷雾组的"第3名后路效应"削弱，不可信信息上不做强动机修正（对齐 STEP 3.5.2 的保守原则）
...
{
  "step": 4 | null,
  "advancement_threshold_points": "int — 本届实时出线门槛（第8名第3名当前积分）",
  "home_third_place_probability": "float | [p_low, p_high] — 真实排名概率（非门槛表）",
  "away_third_place_probability": "float | [p_low, p_high]",
  "home_third_place_rank": "[rank_min, rank_max] — 落第3时的排名区间",
  "away_third_place_rank": "[rank_min, rank_max]",
  "third_place_pool_snapshot": "12组第3名候选积分排序摘要（可解释性）",
  "confidence": "HIGH | MEDIUM | LOW",
  "impact_on_match": {
    "home_desperation_boost": "float（已按 confidence_factor 折损）",
    "away_desperation_boost": "float（已折损）",
    "draw_boost": "float（已折损）",
    "confidence_factor": "1.0 | 0.6 | 0.3"
  }
}
```
...
// v1.5：boost 已在 STEP 4 内部按 confidence_factor 折损（HIGH=1.0/MEDIUM=0.6/LOW=0.3），
//       R2/迷雾组的第3名后路效应自动削弱，此处直接消费无需二次折损
IF step4 != null:
...
// STEP 6 淘汰赛路径修正（v1.4：按 confidence 折损——path_diff 不可信时削弱其概率影响）
IF step6 != null AND step6.knockout_path_bias != 0:
  confidence_factor = 1.0 (HIGH) | 0.6 (MEDIUM) | 0.3 (LOW)
  path_bias = step6.knockout_path_bias * confidence_factor
...
**条件化执行**：R2/R3 均执行（R1 不执行）。**v1.4 重写**：从 v1.1 的"文字引用外部对阵表 + 概率性裸估算"升级为"基于 STEP 0.5 阶段三 `knockout_opponent_graph` 的结构化对手推理"，并新增正式 `confidence` 字段（取代 v1.1 正文里的文字描述）。

**前置依赖**：STEP 0.5 阶段三的 `knockout_opponent_graph` + 本文件「淘汰赛对阵结构参考」内嵌表。

**目标**：回答"本场球队若排第1/第2，R32 首轮遇到谁、对手多强"，量化为 `path_diff` → `SV` → `knockout_path_bias` + `confidence`，喂 STEP 3.5.2 的 SEALED 子态判定与 STEP 5.3 的概率修正。

**推理算法**：

```
1. 落点定位（静态查表）
   查「半区分布速查表」→ 本组第1 → R32 槽位 X + 半区；本组第2 → R32 槽位 Y + 另一半区。

2. 对手实力推理（消费阶段三 opponent_graph，按 opponent_type 分支）
   FOR each 落点 (as_1st=X, as_2nd=Y):
     取 opponent_graph[落点].opponent_type:
       DETERMINED    → opponent_strength = s(opponent_fifa_rank)                 ; conf贡献=HIGH
       CANDIDATE_SET → opponent_strength = s(候选集加权中位 rank)                 ; conf贡献=MEDIUM
       THIRD_POOL    → opponent_strength = s(候选池加权中位 rank)                 ; conf贡献=LOW
                       IF strong_third_risk == true → 路径难度额外 +0.15 上调
   // 归一化 s(rank) = clamp(1 - (rank-1)/80, 0, 1)：rank=1→1.0, rank=40→0.51, rank=80→0.0
   // 小=强；DETERMINED 用精确 rank，CANDIDATE_SET/THIRD_POOL 用候选加权中位 rank

3. 路径偏好量化
   path_diff_as_1st = opponent_strength(as_1st)
   path_diff_as_2nd = opponent_strength(as_2nd)
   SV = path_diff_as_1st - path_diff_as_2nd   // 正值=第1名对手更强=第1路径更难；∈[-1,1]

   IF SV > 0.2:    knockout_path_bias = -0.05 ~ -0.10  // 第1更难 → 倾向第2，不想拿第1
   ELIF SV < -0.2: knockout_path_bias = +0.05 ~ +0.10  // 第2更难 → 倾向第1
   ELSE:           knockout_path_bias = 0.0            // 两路径难度相当
   // 幅度量纲沿用 group_stage_predict.md；阈值 0.2 与 STEP 3.5.2 的 |SV|<0.2 NEUTRAL 判定对齐

4. confidence 定级（木桶效应 + R3 info_clarity 下调）
   base_conf = min(as_1st.conf贡献, as_2nd.conf贡献)   // 取两落点较低者
   IF 本轮 == R3:
     info_clarity = group_index / 11   // 本组在第三轮时间线的位置（见内嵌表）
     IF info_clarity <= 0.5 → base_conf 强制降一档（信息迷雾：其他组本场未赛，落位不透明）
   IF 本组第1/第2归属未锁（seed_locked=false）→ confidence 至多 MEDIUM（名次本身未定）
   confidence = base_conf
```

**轮次要点**（v1.4 修正 v1.1 误导表述）：
- **R2**：大多小组仅打完 1 轮，对方组名次普遍未锁 → opponent_type 以 `CANDIDATE_SET`/`THIRD_POOL` 为主 → confidence 默认 **MEDIUM/LOW**（非 v1.1 所述的精确）。主要用于已 6 分锁定场景的名次价值判断（STEP 3.5）。
- **R3**：本组名次可基于 R1+R2 精算，但**淘汰赛对手的 confidence 取决于 opponent_type + info_clarity**——同步开赛使其他组本场结果未知、第三名身份未定，confidence **非一律 HIGH**。仅当两落点均 DETERMINED 且 info_clarity>0.5 时才达 HIGH。
- **2026 赛制**：使用本文件「淘汰赛对阵结构参考」内嵌表（镜像权威源 `skills/冠亚军分析.md` 2.1-2.5）。
- **2018/2022 赛制**：用该届 R16 对阵表替代内嵌 2026 表，opponent_graph 同理构建（简化为推算小组排名→查对阵表→评估路径难度）。

**输出**：`{ step: 6 | null, path_analysis: { as_1st_match, as_2nd_match, path_diff_as_1st, path_diff_as_2nd, SV, path_preference: "1ST_PREFERRED|2ND_PREFERRED|NEUTRAL", knockout_path_bias, confidence: "HIGH|MEDIUM|LOW", info_clarity } }`
...
```json
{
  ...
  "analysis_trace": { "step1_base": "object", "step2_pattern": "object", "step3_motivation": "object", "step3_5_seeding": "object|null (v1.1, R2/R3)", "step4_third_place": "object|null (v1.5, 含 advancement_threshold_points/home-away_third_place_probability/home-away_third_place_rank/confidence/impact_on_match)", "step6_path": "object|null (v1.4, R2/R3, 含 opponent_graph/path_diff/SV/confidence/info_clarity)" }
}
```
...
| 第3名出线博弈 | 15% | 大幅提升——R3是第3名出线的关键计算轮（v1.5 起为跨组实时排名对比，含实时门槛推算） |
...
# STEP 4: 跳过（输出 null）
# STEP 6: 使用 2018 R16 对阵表（替代内嵌 2026 对阵结构表；阶段三 opponent_graph 同理用 2018 跨组快照构建）
# 特殊修正:
...
tournament_config_override:
  ...
  advancement_rate: 0.500
# STEP 4: 跳过（输出 null）
# STEP 6: 使用 2022 R16 对阵表（替代内嵌 2026 对阵结构表；阶段三 opponent_graph 同理用 2022 跨组快照构建）
# 特殊修正:
...
5. **数据文件依赖（v1.4）**: 全轮次 STEP 0.5 需读取 `data/2026_FIFA_World_Cup_Group_Stage.md`（人工维护的赛程+排名+结果真相源）。**维护责任**：
   - **开赛前一次性**：确认「FIFA排名(主/客)」列与「标注说明」汇总表的 48 队排名完整（含附加赛晋级队），缺失会阻断 STEP 1/2 的 rank_diff 与 STEP 6 对手推理的 `opponent_strength`；
   - **每轮完赛后**：将该组表格的「90分钟」「赛前积分」「赛前排名」更新为官方实际值——这是 R2/R3 预测准确性的硬前提，数据滞后或错误会直接污染 ASS、出线状态、**STEP 0.5 阶段三的跨组出线快照与 opponent_graph**、**STEP 4 的实时第3名门槛与排名对比**、路径分析整条链。
6. **淘汰赛对手推理数据依赖（v1.4 新增）**: R2/R3 的 STEP 6 对手推理依赖两类数据：
   - **静态对阵结构**：本文件「2026 淘汰赛对阵结构参考」内嵌章节，镜像自 `skills/冠亚军分析.md` 2.1-2.5（**权威源**）。对阵结构由 FIFA 抽签固定、不随赛程变化，仅在官方调整对阵时需双向同步。回测 2018/2022 时用该届 R16 对阵表替代。
   - **动态出线快照**：STEP 0.5 阶段三从 data md 的全部 12 组已完成行聚合。data 文件**不含**淘汰赛落位数据、第三轮日期仅日级粒度（同日内两组开赛先后顺序无法从 data 推出），依赖冠亚军分析 2.5 的第三轮时间线 `B→C→A→E→F→D→I→H→G→L→K→J` 计算 `info_clarity`。

---

*基于2018年俄罗斯世界杯和2022年卡塔尔世界杯96场小组赛数据设计，适配2026年美加墨世界杯赛制*
*版本1.5 — 派生自 group_stage_predict v2.1，轮次差异化策略引导；v1.4 打通淘汰赛对手推理链，v1.5 重写第3名出线为跨组实时排名对比*
